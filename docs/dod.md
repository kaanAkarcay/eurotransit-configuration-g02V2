...
- [x] Idempotency level 3 (Kafka): processed_events table in every Kafka consumer (Orders consuming its own order-placed/inventory-reserved/payment-authorized/payment-failed across its four stages, plus Inventory consuming order-failed), keyed on event_id, checked in the same DB transaction as business logic
