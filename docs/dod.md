...
- [x] Consistency model documented in CAP/PACELC terms — strong consistency scoped to `seats.available` (PC/EC: refuse writes under partition, always hit the primary otherwise), with Catalog's display-only availability as the deliberate AP counterpoint and the saga eventually consistent around both. See `consistency-validation.md` §1
...
- [x] Idempotency level 3 (Kafka): processed_events table in every Kafka consumer (Orders' four stage consumers plus Inventory's order-failed consumer), keyed on event_id, claimed with INSERT ... ON CONFLICT DO NOTHING in the same DB transaction as the business logic it guards (`consistency-validation.md` §3)
